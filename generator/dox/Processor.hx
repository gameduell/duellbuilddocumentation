...
class Processor {
	...
	function filter(root:TypeRoot) {
		...
		if (config.toplevelPackage != "") {
			var found = false;
			function filter(toplevelFilter: String, tree: TypeTree) {
				switch (tree) {
					case TPackage(name, full, subs):
						var split2 = toplevelFilter.split(".");
						if (split2[0] != name) {
							return;
						}
						split2.shift();
						...
						subs.iter(filter.bind(split2.join(".")));
					case _:
				}
			}
	...
	function removeEnumAbstractCast(field:ClassField)
	{
		...
		if (field.type.match(CAbstract(_, _)) &&
			...
			field.get == RInline && field.set == RNo && field.expr != null &&
