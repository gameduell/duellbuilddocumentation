...
class Template {
	...
	**/
	public function new(i:Input, ?sourceName = null) {
		byteData = byte.ByteData.ofString(i.readAll().toString());
	...
	**/
	static public function fromString(s:String, ?sourceName = null) {
		return new Template(new StringInput(s), sourceName);
	}
	...
	**/
	static public function fromFile(path:String) {
		...
		return new Template(File.read(path), p.file + "." + p.ext);
	}
	#end
